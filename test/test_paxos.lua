...
p = paxos_module({ acceptor_timeout = 1, proposer_timeout = 1 })
s = mock_storage()
a = spawn(function()
...
m = spawn(function()
            ...
            ok, err, res = p.propose(q, { a }, 'x')
            -- tdump("propose-done", p.stats())
            assert(ok and not err and res)
            assert(seq_eq(res.seq, q))
            assert(res.val == 'x')
            done = done + 1
          end)
repeat
...
spawn(function()
        ...
        ok, err, res = p.propose(q1, { a }, 'x')
        -- tdump("propose-done", p.stats())
        assert(ok and not err and res)
        assert(seq_eq(res.seq, q1))
        assert(res.val == 'x')
        done = done + 1
        spawn(function()
                q2 = p.seq_mk(2, ambox.self_addr())
                ok, err, res = p.propose(q2, { a }, 'y')
                -- tdump("propose-done", p.stats())
                assert(ok and not err and res)
                assert(seq_eq(res.seq, q2))
                assert(res.val == 'y')
                done = done + 1
              end)
      end)
...
spawn(function()
        ...
        spawn(function()
                ...
                spawn(function()
                        ...
                        ok, err, res = p.propose(q3, { a }, 'z')
                        -- tdump("propose-done", p.stats())
                        assert(ok and not err and res)
                        assert(seq_eq(res.seq, q3))
                        assert(res.val == 'z')
                        done = done + 1
                      end)
              end)
...
print("test - propose 2 values, 2nd rejected, 3rd wins, flat")
done = 0
p = paxos_module({ acceptor_timeout = 1, proposer_timeout = 1 })
q1 = nil
q2 = nil
